...
lazy_static! {
    static ref PRINTER: Arc<Mutex<Box<dyn Printer>>> = match env::var("FIREHOSE_ENABLED") {
        Ok(v) if v == "true" => Arc::new(Mutex::new(Box::new(StdoutPrinter {}))),
        _ => Arc::new(Mutex::new(Box::new(DiscardPrinter {}))),
    };
}

pub fn print(input: String) {
    PRINTER.lock().unwrap().print(&input);
}
...
impl Default for Config {
    fn default() -> Self {
        Config { instrumentation: Instrumentation::None }
    }
}
...
impl Context {
    pub fn new(instrumentation: Instrumentation) -> Context {
        Context { instrumentation, printer: Arc::new(Box::new(StdoutPrinter {})) }
    }
    ...
    pub fn is_finalize_block_enabled(&self) -> bool {
        return self.instrumentation == Instrumentation::Full
            || self.instrumentation == Instrumentation::BlockProgress;
    }

    pub fn init(&self) {
        // let platform_version = to_deepmind_version();
        // FIXME: Extract from crate (this one maybe?)
        let platform_version = (1, 2, 0);

        self.printer.print(format!("INIT {protocol_major} {protocol_minor} {platform_major} {platform_minor} {platform_patch}",
