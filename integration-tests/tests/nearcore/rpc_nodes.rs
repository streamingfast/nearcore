...
fn test_query_rpc_account_view_account_doesnt_exist_must_return_error() {
    ...
    cluster.exec_until_stop(|_, rpc_addrs, _| async move {
        ...
        let error_message = query_response
            ...
            loop {
                ...
                if let Ok(Ok(block)) = res {
                    if block.header.height > 10 {
                        ...
                        match handler_error {
                            methods::broadcast_tx_commit::RpcTransactionError::InvalidTransaction {
                                context
                            } => assert_eq!(
                                    context,
                                    near_primitives::errors::InvalidTxError::NotEnoughBalance {
                                        signer_id: "near.0".parse().unwrap(),
                            ...
                            _ => panic!("expected an InvalidTransaction error, got otherwise")
                        }
                        System::current().stop();
                        break;
                    }
            ...
            loop {
                ...
                if let Ok(Ok(block)) = res {
                    if block.header.height > 10 {
                        ...
                        match handler_error {
                            methods::EXPERIMENTAL_broadcast_tx_sync::RpcTransactionError::RequestRouted {
                                transaction_hash
                            } => assert_eq!(
                                transaction_hash,
                                request.signed_transaction.get_hash()
                            ),
                            _ => panic!("expected a RequestRouted error, got otherwise")
                        }
                        System::current().stop();
                        break;
                    }
            ...
            loop {
                ...
                if let Ok(Ok(block)) = res {
                    if block.header.height > 10 {
                        ...
                        match handler_error {
                            methods::EXPERIMENTAL_broadcast_tx_sync::RpcTransactionError::UnknownTransaction {
                                requested_transaction_hash
                            } => assert_eq!(
                                requested_transaction_hash,
                                transaction.get_hash()
                            ),
                            _ => panic!("expected an UnknownTransaction error, got otherwise")
                        }
                        System::current().stop();
                        break;
                    }
                ...
                if let Ok(Ok(block)) = res {
                    if block.header.height > 10 {
                        ...
                        assert!(
                            matches!(
                                handler_error,
                                methods::EXPERIMENTAL_broadcast_tx_sync::RpcTransactionError::DoesNotTrackShard
                            ),
                            "expected a DoesNotTrackShard error, got otherwise"
                        );
                        System::current().stop();
                        break;
                    }
